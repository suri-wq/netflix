...
const MoviePage = () => {
  ...
  const [filters, setFilters] = useState({ genre :null, rating:null, popularity:null})
  ...
  const filteredAndSortedResults = (data?.results || []).filter(
        ...
        <Col lg={12} xs={12} style={{marginBottom:'20px'}}>
          <ButtonGroup>
            <DropdownButton variant='danger' as={ButtonGroup} title="인기도" id="bg-nested-dropdown">
              ...
              <Dropdown.Item eventKey="2" onClick={()=> setFilters(prev => ({ ...prev, popularity: 'asc', rating:null}))}>인기 적은 순</Dropdown.Item>
            </DropdownButton>
            <DropdownButton variant='danger' as={ButtonGroup} title="장르" id="bg-nested-dropdown">
              {movieGenreData?.map((genre)=>
                <Dropdown.Item
                 key={genre.id}
                 onClick={()=>setFilters(prev=>({...prev, genre:genre.id}))}
                 >{genre.name}
                 </Dropdown.Item>
              )}

            </DropdownButton>
            <DropdownButton variant='danger' as={ButtonGroup} title="평점" id="bg-nested-dropdown">
              <Dropdown.Item onClick={() => setFilters(prev => ({ ...prev, rating: 'desc', popularity: null }))}>
                평점 높은 순
              </Dropdown.Item>
              <Dropdown.Item onClick={() => setFilters(prev => ({ ...prev, rating: 'asc', popularity: null }))}>
                평점 낮은 순
              </Dropdown.Item>
            </DropdownButton>
