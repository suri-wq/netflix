...
const MovieDetailPage = () => {
  ...
  if (
    ...
    console.log('로딩 중 video 상태:', video)
    return (
      <div>
        <Spinner
          animation="border"
  ...
  if (isError || reviewError || recError || vidError || creditsError) {
    return (
      <Alert variant="danger">
        {error?.message ||
          reviewErrMsg?.message ||
          recErrMsg?.message ||
          vidErrMsg?.message ||
          creditsErrMsg?.message ||
          '알 수 없는 에러'}
      </Alert>
    )
  }
  ...
  console.log('ccc', credits.name)

  return (
    <Container>
      <Row>
        <Col lg={12} xs={12} className="text-center backdrop-img">
          <img
        ...
        <Col lg={6} xs={12} className="mb-4 mb-lg-0">
          <div>
            ...
            <div className="mb-2 text-white">
              <span>
                <strong>평균</strong> {movie.vote_average}
              </span>{' '}
            ...
            <div className="text-white">
              <strong>출연</strong>
              {credits.slice(0, 5).map((name, index) => (
                <span key={index}>
                  {' '}
                  {name.name} {index < 4 && ' | '}
                </span>
            ...
            <Accordion alwaysOpen>
              {review.results.map((rvww, index) => (
                <Accordion.Item key={rvww.id || index} eventKey={String(index)}>
                  <Accordion.Header>
                    <strong>name : </strong> {rvww.author}
                  </Accordion.Header>
                  <Accordion.Body>
      ...
      <Row>
        ...
        <Col>
          <MovieSlider
            title=""
            movies={movie_recommendations.results}
